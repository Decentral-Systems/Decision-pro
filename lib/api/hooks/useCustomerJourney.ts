...
export function useCustomerJourneyInsights(filters?: JourneyStatisticsFilters) {
  ...
  return useQuery<CustomerJourneyInsights | null>({
    ...
    queryFn: async () => {
      try {
        const rawData = await apiGatewayClient.getCustomerJourneyInsights(filters);
...
export function useCustomerJourneyTimeline(
  ...
  return useQuery<CustomerJourneyTimelineItem[]>({
    ...
    queryFn: async () => {
      ...
      try {
        const data = await apiGatewayClient.getCustomerJourneyTimeline(customerId, filters);
        
        // Ensure data is properly transformed to CustomerJourneyTimelineItem[]
        if (Array.isArray(data)) {
          return data.map((item: any) => ({
            eventId: item.eventId || item.event_id || item.id || String(item.timestamp),
            ...
            subStage: item.subStage || item.sub_stage || "onboarding.customer_created",
            action: item.action || "",
    ...
    enabled: isAuthenticated && tokenSynced && !!session?.accessToken && !!customerId,
...
export function useTopCustomers(limit: number = 10, sortBy: "credit_score" | "revenue" | "loan_amount" = "credit_score") {
  const { isAuthenticated, tokenSynced, session } = useAuth();
