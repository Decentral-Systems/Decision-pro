...
import { ProductRecommendationsResponse, RecommendationStatistics, ProductRecommendation } from "@/types/product-intelligence";
...
export function useProductRecommendations(customerId?: string, limit: number = 10) {
  ...
  return useQuery<ProductRecommendationsResponse | null>({
    ...
    queryFn: async () => {
      try {
        const rawData = await apiGatewayClient.getProductRecommendations(customerId, limit);
        ...
        } else if (rawData && typeof rawData === "object" && "recommendations" in rawData) {
          recommendationsArray = (rawData as any).recommendations || [];
        }
        
        // Normalize and validate recommendations (transforms to camelCase)
        const normalizedRecommendations = normalizeResponseFormat<ProductRecommendation[]>(
...
export function useRecommendationStats(dateParams?: { start_date: string; end_date: string }) {
  const { isAuthenticated, tokenSynced, session } = useAuth();
  ...
  return useQuery<RecommendationStatistics | null>({
    ...
    queryFn: async () => {
      try {
        const rawData = await apiGatewayClient.getRecommendationStats(dateParams);
