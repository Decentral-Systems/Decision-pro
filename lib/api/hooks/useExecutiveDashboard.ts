...
export function useExecutiveDashboardData(dateParams?: {
  start_date: string;
  end_date: string;
}) {
  ...
  return useQuery<ExecutiveDashboardData | null>({
    ...
    queryFn: async () => {
      try {
        ...
        console.log(
          "[useExecutiveDashboardData] Fetching executive dashboard data",
          {
            dateParams,
            correlationId,
            isAuthenticated,
        ...
        const rawData =
          await apiGatewayClient.getExecutiveDashboardData(dateParams);
        ...
        if (
          ...
          console.warn(
            "[useExecutiveDashboardData] Raw data is empty or null",
            {
              rawData,
              rawDataType: typeof rawData,
            }
        ...
        if (!transformedData) {
          console.warn(
            "[useExecutiveDashboardData] Failed to transform executive dashboard data",
            {
              rawData,
        ...
        console.error(
          "[useExecutiveDashboardData] Error fetching executive dashboard data",
          {
            statusCode,
        ...
        if (statusCode === 401 || statusCode === 404) {
          console.warn(
            "[useExecutiveDashboardData] Executive dashboard data not available:",
            statusCode
          );
          return null;
        }
...
export function useExecutiveDashboardCacheMetadata() {
  const metadata =
    typeof window !== "undefined"
      ? getCacheMetadata("executive-dashboard")
      : null;
  return metadata;
}
