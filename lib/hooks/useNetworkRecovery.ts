 ...
 * 
 * Monitors network connectivity and API health, automatically recovering
 * failed queries when network comes back online. This prevents queries
 * from getting stuck in error states.
 */
...
/**
 * Hook to monitor network status and automatically recover failed queries
 * 
 * This hook:
 * 1. Monitors API health status
 * 2. Detects when network comes back online after being offline
 * 3. Automatically invalidates all failed queries to trigger refetch
 * 4. Prevents queries from getting stuck in error states
 */
export function useNetworkRecovery(options: UseNetworkRecoveryOptions = {}) {
  ...
  useEffect(() => {
    ...
    if (wasOffline && isNowOnline) {
      console.log("[NetworkRecovery] Network recovered, invalidating failed queries");
      ...
      recoveryTimeoutRef.current = setTimeout(() => {
        try {
          ...
          if (queriesToRecover.length > 0) {
            console.log(`[NetworkRecovery] Recovering ${queriesToRecover.length} queries`);
