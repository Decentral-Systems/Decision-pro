import { ethiopianIdSchema, ethiopianPhoneSchema } from "@/lib/utils";
import {
...
import { z } from "zod";
import { NBE_RULES } from "../constants/scoring.constant";

export const creditScoringFormSchema = z
  .object({
    // Basic Info
    ...
    loan_amount: z
      ...
      .min(
        NBE_RULES.min_loan_amount,
        `Loan amount must be at least ${NBE_RULES.min_loan_amount.toLocaleString()} ETB (NBE minimum)`
      )
      .max(
        NBE_RULES.max_loan_amount,
        `Loan amount cannot exceed ${NBE_RULES.max_loan_amount.toLocaleString()} ETB (NBE maximum)`
      ),
    loan_term_months: z
      .number()
      .int()
      ...
      .max(
        NBE_RULES.max_loan_term_months,
        `Loan term cannot exceed ${NBE_RULES.max_loan_term_months} months (NBE maximum)`
      ),
  ...
  .refine(
    (data) => {
      ...
      const maxAffordablePayment =
        data.monthly_income * NBE_RULES.salary_rule_ratio;
      const proposedPayment = data.loan_amount / data.loan_term_months;
      return proposedPayment <= maxAffordablePayment;
    },
    {
      message:
        "Loan payment exceeds 1/3 of monthly income (NBE compliance rule). Please reduce loan amount or extend term.",
      path: ["loan_amount"],
    }
  )
  .refine(
    (data) => {
      ...
      const maxAffordableLoan =
        data.monthly_income *
        NBE_RULES.salary_rule_ratio *
        data.loan_term_months;
      return data.loan_amount <= maxAffordableLoan;
    },
    {
      message:
        "Loan amount exceeds maximum affordable amount based on income and term (NBE compliance)",
      path: ["loan_amount"],
    }
  )
  .refine(
  ...
  .refine(
    (data) => {
      ...
      if (
        ...
        data.credit_utilization_ratio !== undefined
      ) {
  ...
  .refine(
    (data) => {
      ...
      if (
        data.monthly_income &&
        (data.monthly_income < 100 || data.monthly_income > 1000000)
      ) {
        return false;
      }
      return true;
