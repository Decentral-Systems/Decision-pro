...
export function MarketRiskWidget({ executiveData }: MarketRiskWidgetProps) {
  const { data: marketRisk, isLoading, error: marketRiskError, refetch: refetchMarketRisk, isError: isMarketRiskError } = useMarketRiskAnalysis();
  const [timeRange, setTimeRange] = useState<"30d" | "90d" | "1y">("90d");
  
  // Get historical data and sectors from API
  const days = timeRange === "30d" ? 30 : timeRange === "90d" ? 90 : 365;
  const { data: historicalApiData, isLoading: isLoadingHistorical, error: historicalError, refetch: refetchHistorical } = useMarketRiskHistorical(days);
  const { data: sectorsApiData, isLoading: isLoadingSectors, error: sectorsError, refetch: refetchSectors } = useMarketRiskSectors();
  ...
  const concentrationRisk = executiveData?.portfolio_health?.concentration_risk 
  ...
  const historicalData = useMemo(() => {
    if (historicalApiData && Array.isArray(historicalApiData) && historicalApiData.length > 0) {
      // Transform API data to chart format
      return historicalApiData.map((item: any) => ({
  ...
  const sectorBreakdown = useMemo(() => {
    if (sectorsApiData && Array.isArray(sectorsApiData) && sectorsApiData.length > 0) {
      // Transform API data to chart format
  ...
  const recommendations = useMemo(() => {
    ...
    if (concentrationRisk >= 30) {
      recs.push("High concentration risk detected - consider portfolio diversification");
      ...
      recs.push("Medium concentration risk - monitor sector allocations closely");
      recs.push("Consider gradual diversification to reduce concentration");
    } else {
      recs.push("Portfolio shows good diversification - maintain current allocation strategy");
    ...
    return (
      <Card>
        <CardHeader>
          ...
          <CardDescription>Economic conditions and market impact</CardDescription>
        </CardHeader>
        <CardContent>
        ...
        <CardContent>
          <Alert variant="destructive">
            ...
            <AlertDescription className="flex items-center justify-between">
              <div>
                <span className="font-semibold">Failed to load market risk analysis.</span>
    ...
    <Card>
      <CardHeader>
        <div className="flex items-center justify-between">
          <div>
            ...
            <CardDescription>Economic conditions and market impact on portfolio</CardDescription>
          </div>
          <ApiStatusIndicator 
      ...
      <CardContent>
        <div className="space-y-6">
          {/* Market Conditions */}
          <div>
            <h4 className="text-sm font-semibold mb-3">Market Conditions</h4>
            <div className="grid grid-cols-2 gap-4">
              <div className="flex items-center justify-between p-3 border rounded-lg">
                <span className="text-sm text-muted-foreground">Economic Indicator</span>
                <Badge
                  variant={
          ...
          {/* Portfolio Impact */}
          <div>
            <h4 className="text-sm font-semibold mb-3">Portfolio Impact</h4>
            <div className="space-y-2 p-4 bg-muted rounded-lg">
              <div className="flex items-center justify-between">
                <span className="text-sm text-muted-foreground">Estimated Default Rate</span>
                <span className="font-semibold text-red-600">
                  {portfolio_impact?.estimated_default_rate?.toFixed(2) || "0.00"}%
                </span>
              </div>
              <div className="flex items-center justify-between">
              ...
              <div className="mt-3 pt-3 border-t">
                ...
                <p className="text-sm text-muted-foreground">
                  {portfolio_impact?.recommended_strategy || "No specific strategy recommended"}
                </p>
              </div>
            </div>
          </div>
          ...
          {/* Economic Factors */}
          {economic_factors && Array.isArray(economic_factors) && economic_factors.length > 0 && (
            <div>
              <h4 className="text-sm font-semibold mb-3">Key Economic Factors</h4>
          ...
          <div className="pt-4 border-t">
            <h4 className="text-sm font-semibold mb-3">Historical Risk Trends ({timeRange})</h4>
            {isLoadingHistorical ? (
              <Skeleton className="h-48 w-full" />
              ...
              <Alert>
                ...
                <AlertDescription>
                  Historical risk trend data is not available for the selected time range.
                </AlertDescription>
              </Alert>
            )}
          ...
          <div className="pt-4 border-t">
            ...
            {isLoadingSectors ? (
              ...
              <Alert>
                <Info className="h-4 w-4" />
                <AlertDescription>
                  Sector breakdown data is not available. This may be due to insufficient portfolio data.
                </AlertDescription>
              </Alert>
            )}
          </div>
          ...
          {concentrationRisk !== undefined && (
            <div className="pt-4 border-t">
              <h4 className="text-sm font-semibold mb-3">Portfolio Concentration Risk</h4>
              <div className="space-y-2">
                <div className="flex items-center justify-between p-3 border rounded-lg">
                  <span className="text-sm text-muted-foreground">Concentration Risk Score</span>
                  <Badge variant={concentrationRisk < 20 ? "default" : concentrationRisk < 30 ? "secondary" : "destructive"}>
                    {formatPercentage(concentrationRisk / 100)} (Higher = More Concentrated)
                  </Badge>
                </div>
                <div className="text-xs text-muted-foreground">
                  Based on portfolio diversification analysis. Lower scores indicate better diversification.
                </div>
              </div>
            </div>
          )}
