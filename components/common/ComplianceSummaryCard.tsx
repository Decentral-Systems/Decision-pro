 ...
 * Real-time NBE compliance status display
 */
...
import { NBEComplianceResult } from "@/lib/utils/nbe-compliance";
...
export function ComplianceSummaryCard({
  ...
  const complianceScore = compliance.compliant ? 100 : Math.max(0, 100 - (compliance.violations.length * 20));
        ...
        {Object.keys(compliance.recommendations).length > 0 && (
          <div className="pt-2 border-t space-y-1">
            ...
            {compliance.recommendations.max_monthly_payment && (
              <div className="text-xs text-muted-foreground">
                Max Payment: {compliance.recommendations.max_monthly_payment.toLocaleString()} ETB/month
              </div>
            )}
          </div>
        )}
