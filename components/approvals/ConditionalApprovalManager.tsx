...
export function ConditionalApprovalManager({
  ...
  const fulfilledConditions = conditions.filter(c => c.status === "fulfilled");
  const overdueConditions = conditions.filter(c => c.status === "overdue" || isOverdue(c.deadline));
            ...
            <DialogContent className="max-w-2xl">
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <Select value={newCondition.type} onValueChange={(value) => setNewCondition(prev => ({ ...prev, type: value }))}>
                    <SelectTrigger>
                ...
                <div>
                  <Label htmlFor="condition-deadline">Deadline (Optional)</Label>
                  <Input
                    id="condition-deadline"
                    type="date"
                    value={newCondition.deadline}
                    onChange={(e) => setNewCondition(prev => ({ ...prev, deadline: e.target.value }))}
                  />
                </div>
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-3 gap-4">
              <div className="text-center p-4 bg-yellow-50 dark:bg-yellow-900/20 rounded-lg">
                <div className="text-2xl font-bold text-yellow-600">{pendingConditions.length}</div>
                <div className="text-sm text-muted-foreground">Pending</div>
              </div>
              <div className="text-center p-4 bg-green-50 dark:bg-green-900/20 rounded-lg">
                <div className="text-2xl font-bold text-green-600">{fulfilledConditions.length}</div>
                <div className="text-sm text-muted-foreground">Fulfilled</div>
              </div>
              <div className="text-center p-4 bg-red-50 dark:bg-red-900/20 rounded-lg">
                <div className="text-2xl font-bold text-red-600">{overdueConditions.length}</div>
              ...
              {conditions.map((condition) => (
                <div
                  ...
                  className={`p-4 border rounded-lg ${
                    condition.status === "overdue" || isOverdue(condition.deadline)
                    ...
                    <div className="flex-1">
                      <div className="flex items-center gap-2 mb-2">
                        ...
                        <Badge variant="outline">
                          {conditionTypes.find(t => t.value === condition.type)?.label || condition.type}
                        </Badge>
                      </div>
                      <p className="text-sm font-medium mb-2">{condition.description}</p>
                      {condition.deadline && (
                      ...
                      {condition.required_documents && condition.required_documents.length > 0 && (
                        <div className="text-xs text-muted-foreground mb-2">
                          Required documents: {condition.required_documents.join(", ")}
                        </div>
                      )}
                      {condition.fulfilled_date && (
                        <p className="text-xs text-green-600">
                          Fulfilled on: {new Date(condition.fulfilled_date).toLocaleDateString()}
                        </p>
                      )}
                      {condition.notes && (
              ...
              <div className="p-4 bg-muted rounded-lg">
                <div className="flex items-center justify-between mb-2">
                  ...
                  <span className="text-sm text-muted-foreground">
                    {fulfilledConditions.length} of {conditions.length} conditions fulfilled
                  </span>
                </div>
