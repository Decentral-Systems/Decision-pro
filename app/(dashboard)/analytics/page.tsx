...
import { useAnalyticsData, useRiskDistribution, useApprovalRates, usePortfolioMetrics, usePreviousPeriodMetrics } from "@/lib/api/hooks/useAnalytics";
...
const MetricTrend = dynamic(() => import("@/components/charts/MetricTrend").then(mod => ({ default: mod.MetricTrend })), {
...
function AnalyticsPageContent() {
  ...
  usePageShortcuts([
    {
      ...
      action: () => {
        // Export data
        const exportButton = document.querySelector('button:has-text("Export")') as HTMLButtonElement;
  ...
  const [timeframe, setTimeframe] = useState<"daily" | "weekly" | "monthly">(
    (searchParams.get("timeframe") as "daily" | "weekly" | "monthly") || "monthly"
  );
  ...
  const { data: analyticsDataFromAPI, isLoading: analyticsLoading, error: analyticsError, refetch: refetchAnalytics } = useAnalyticsData({
    time_range: timeframe,
  });
  const { data: portfolioMetrics, isLoading: portfolioLoading, refetch: refetchPortfolio } = usePortfolioMetrics({
    time_range: timeframe,
  });
  const { data: riskDistribution, isLoading: riskLoading, refetch: refetchRisk } = useRiskDistribution();
  const { data: approvalRates, isLoading: approvalLoading, refetch: refetchApproval } = useApprovalRates(timeframe);
  const { data: previousPeriodData, isLoading: previousPeriodLoading } = usePreviousPeriodMetrics(timeframe);
  
  // Get cache metadata for last-updated timestamp (only on client after mount)
  const analyticsCacheMetadata = isMounted ? getCacheMetadata("analytics") : null;
  
  const isLoading = analyticsLoading || portfolioLoading || riskLoading || approvalLoading || previousPeriodLoading;
  const error = analyticsError;
  const hasAnyData = analyticsDataFromAPI || portfolioMetrics || riskDistribution || approvalRates;

  // Combine real data from multiple sources - no fallback data
  const analyticsData = {
    portfolio_metrics: portfolioMetrics || analyticsDataFromAPI?.portfolio_metrics || {
      total_loans: 0,
      total_exposure: 0,
      average_loan_size: 0,
    ...
    risk_distribution: ensureArray(riskDistribution || analyticsDataFromAPI?.risk_distribution),
    approval_rates: ensureArray(approvalRates || analyticsDataFromAPI?.approval_rates),
  ...
  const calculateDelta = (current: number, previous: number) => {
    if (previous === 0) return { value: current, percentage: current > 0 ? 100 : 0, trend: current > 0 ? "up" : "neutral" };
    ...
    <div className="space-y-8">
      <div className="flex items-center justify-between">
            ...
            <span>
              ...
              {isMounted && analyticsCacheMetadata?.lastUpdated && (
                <span className="ml-2 text-xs">
                  • Last updated: {formatLastUpdated(analyticsCacheMetadata.lastUpdated)}
                </span>
              )}
            </span>
          </div>
        ...
        <div className="flex items-center gap-2">
            ...
            onClick={async () => {
              try {
                const exportData = [
                  ...
                  ...(Array.isArray(analyticsData.risk_distribution) ? analyticsData.risk_distribution.map((r: any) => ({
                    Category: r.category,
                    Count: r.count,
                    Percentage: r.percentage,
                    Exposure: r.total_exposure,
                  })) : []),
                  ...(Array.isArray(analyticsData.approval_rates) ? analyticsData.approval_rates.map((a: any) => ({
                    Period: a.period,
          ...
          <ApiStatusIndicator endpoint="/api/analytics" label="Live" showResponseTime={true} />
          <KeyboardShortcutsDialog />
          <DashboardCustomization 
            page="analytics"
            availableSections={["Portfolio Metrics", "Revenue & Customer Analytics", "Risk Distribution", "Approval Rates"]}
          />
          <Button
            variant="outline"
            size="sm"
          ...
          >
            ...
            {comparisonPeriods.length > 0 ? "Disable Comparison" : "Enable Comparison"}
          </Button>
        </div>
      </div>
      ...
      {error && !hasAnyData && (
        <Alert variant="destructive">
          ...
          <AlertDescription className="flex items-center justify-between">
            <div>
              <div>
                <span className="font-semibold">Failed to load analytics data from API.</span>
            ...
            <Card>
              ...
              <CardContent>
                <div className="text-2xl font-bold">
                  {formatCurrency(analyticsData.portfolio_metrics.total_exposure)}
                </div>
                <div className="flex items-center gap-2 mt-1">
                  ...
                  <span className={`text-xs ${exposureDelta.trend === "up" ? "text-green-600" : exposureDelta.trend === "down" ? "text-red-600" : "text-muted-foreground"}`}>
                    {exposureDelta.trend === "up" ? "+" : exposureDelta.trend === "down" ? "-" : ""}
            ...
            <Card>
              ...
              <CardContent>
                ...
                <div className="flex items-center gap-2 mt-1">
                  ...
                  <span className={`text-xs ${scoreDelta.trend === "up" ? "text-green-600" : scoreDelta.trend === "down" ? "text-red-600" : "text-muted-foreground"}`}>
                    {scoreDelta.trend === "up" ? "+" : scoreDelta.trend === "down" ? "-" : ""}
        ...
        <Card>
          <CardHeader className="flex items-center justify-between">
            ...
            <Button
              ...
              size="sm"
              onClick={() => {
                const exportData = analyticsData.customer_segments.map((seg: any) => ({
                  Segment: seg.segment,
                  Count: seg.count,
              ...
              disabled={!analyticsData.customer_segments || analyticsData.customer_segments.length === 0}
            >
              <Download className="h-4 w-4" />
            </Button>
          </CardHeader>
          <CardContent>
            {analyticsData.customer_segments && analyticsData.customer_segments.length > 0 ? (
              <SunburstChart
      ...
      >
        ...
        <CardContent>
          <div className="space-y-4">
            {Array.isArray(analyticsData.risk_distribution) && analyticsData.risk_distribution.length > 0 ? (
              analyticsData.risk_distribution.map((risk: any) => (
              <div key={risk.category} className="space-y-2">
                <div className="flex items-center justify-between">
                  <span className="text-sm font-medium capitalize">{risk.category} Risk</span>
      ...
      <Card>
        ...
        <CardContent>
          <div className="space-y-4">
            {Array.isArray(analyticsData.approval_rates) && analyticsData.approval_rates.length > 0 ? (
