...
import { CreateUserRequest, UpdateUserRequest, User } from "@/types/admin";
...
export default function UserManagementPage() {
  ...
  const [viewingActivityUserId, setViewingActivityUserId] = useState<
    string | null
  >(null);
  ...
  const canModifyUser = (targetUser: User) => {
    ...
    if (
      ...
      targetUser.roles?.includes(UserRole.ADMIN)
    ) {
      return false;
    }
    return true;
  ...
  const requiresDualApproval = (user: User, newRoles: string[]) => {
    ...
    const wasAdmin = user.roles?.includes(UserRole.ADMIN);
  ...
  const handleUpdateUser = async (data: UpdateUserRequest) => {
    ...
    if (data.roles && requiresDualApproval(editingUser, data.roles)) {
      const confirmed = confirm(
        "Admin role changes require dual approval. Continue?"
      );
      if (!confirmed) return;
    }
  ...
  return (
    <div className="space-y-8">
      ...
      {!isLoading &&
        !error &&
        (data === null || !data || (data.items && data.items.length === 0)) && (
          <Alert>
            <AlertTriangle className="h-4 w-4" />
            <AlertDescription>
                ...
                <div className="space-y-2">
                  ...
                  <Select value={roleFilter} onValueChange={setRoleFilter}>
                    ...
                    <SelectContent>
                      ...
                      <SelectItem value="credit_analyst">
                        Credit Analyst
                      </SelectItem>
        ...
        <Card>
          ...
          <CardContent>
                  ...
                  <Button
                    ...
                    onClick={async () => {
                      const newRole = prompt(
                        "Enter new role for selected users:"
                      );
                      if (!newRole) return;
            ...
            <UsersTable
              ...
              onDelete={(userId) => {
                const user = (data as any)?.items?.find(
                  (u: User) => u.user_id === userId
                );
        ...
        <UserActivityDialog
          ...
          userName={
            (data as any)?.items?.find(
              (u: User) => u.user_id === viewingActivityUserId
            )?.username
          }
