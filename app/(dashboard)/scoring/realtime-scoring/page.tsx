...
export default function RealtimeScoringPage() {
  ...
  // Advanced filtering
  const { filters, setFilters, applyFilters, hasActiveFilters } =
    useRealtimeFilters();
  ...
  } = useRealtimeDashboard(searchCustomerId || undefined);
  const {
    data: scoreFeed,
  ...
  useEffect(() => {
    if (realtimeScoreUpdate && !isPaused) {
      ...
      if (
        realtimeScoreUpdate.risk_category === "very_high" ||
      ...
      alternativeFeed.forEach((item: any) => {
        const score: RealtimeScoreEntry = {
          ...
          risk_category: (item.risk_category ||
            item.riskCategory ||
            "medium") as RealtimeScoreEntry["risk_category"],
          timestamp:
  ...
  return (
    <div className="space-y-8">
          ...
          <AlertDescription className="flex items-center justify-between">
            <div>
              <span className="font-semibold">
                Failed to load real-time metrics from API.
              </span>
              <p className="mt-1 text-sm text-muted-foreground">
                Error:{" "}
                {(metricsError as any)?.message || "Unknown error occurred"}
                {(metricsError as any)?.statusCode &&
                  ` (Status: ${(metricsError as any)?.statusCode})`}
              </p>
            </div>
            <Button
              variant="outline"
      ...
      {feedError && (
        <Alert variant="destructive">
          ...
          <AlertDescription className="flex items-center justify-between">
            <div>
              <span className="font-semibold">
                Failed to load score feed from API.
              </span>
              <p className="mt-1 text-sm text-muted-foreground">
                Error: {(feedError as any)?.message || "Unknown error occurred"}
                {(feedError as any)?.statusCode &&
                  ` (Status: ${(feedError as any)?.statusCode})`}
              </p>
            </div>
            <Button
              variant="outline"
      ...
      {!metricsLoading &&
        ...
        !feedError &&
        !metrics &&
        (!scoreFeed || scoreFeed.length === 0) &&
        liveScoreFeed.length === 0 && (
          <Alert>
            <AlertCircle className="h-4 w-4" />
            <AlertDescription>
              No real-time scoring data available. The API returned an empty
              result.
            </AlertDescription>
          </Alert>
        )}
              ...
              <DropdownMenuContent align="end">
                <DropdownMenuItem
                  onClick={async () => {
                    ...
                    const exportData = displayScores
                      .slice(0, 100)
                      .map((score) => ({
                    ...
                    onClick={async () => {
                      ...
                      const exportData = displayScores
                        ...
                        .map((score) => ({
                          ...
                          "Loan Amount": score.loan_amount || "N/A",
                          "Correlation ID":
                            score.correlation_id || correlationId,
                        }));
                      await exportToCSV(
                        exportData,
                        "realtime_scoring_feed",
              ...
              <div className="space-y-2">
                ...
                {streamLatencies.length > 0 && (
                  <div className="text-xs text-muted-foreground">
                    ...
                    P95:{" "}
                    {Math.round(
                      streamLatencies.sort((a, b) => b - a)[
                        Math.floor(streamLatencies.length * 0.05)
                      ] || 0
                    )}
                    ms
                  </div>
      ...
      >
        <div className="space-y-2">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              {hasActiveFilters &&
                allScores.length !== displayScores.length && (
                  <div className="text-sm text-muted-foreground">
                    Showing {displayScores.length} of {allScores.length} scores
                    (filtered)
                  </div>
                )}
          ...
          <ScoreFeed
            scores={displayScores.slice(0, windowSize)}
            maxEntries={windowSize}
          />
        </div>
      </DashboardSection>
    </div>
