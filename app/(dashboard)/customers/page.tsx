...
import { useCustomersList, useExportCustomers } from "@/lib/api/hooks/useCustomers";
...
import { Plus, Download, Filter, X, Users, BarChart3, AlertTriangle, RefreshCw, Calendar, Shield, DollarSign, AlertCircle, Activity, List } from "lucide-react";
import { DashboardSection } from "@/components/dashboard/DashboardSection";
import { Alert, AlertDescription, AlertTitle } from "@/components/ui/alert";
import { Skeleton } from "@/components/ui/skeleton";
import { useToast } from "@/hooks/use-toast";
import { ErrorBoundary } from "@/components/ErrorBoundary";
import { exportToCSV, exportToExcel, exportToPDF, generateCustomersPDF } from "@/lib/utils/exportHelpers";
...
const CustomersTable = dynamic(() => import("@/components/data-table/CustomersTable").then(mod => ({ default: mod.CustomersTable })), {
  loading: () => <Skeleton className="h-96 w-full" />,
  ssr: false,
});

const CustomerCreationDialog = dynamic(() => import("@/components/customer/CustomerCreationDialog").then(mod => ({ default: mod.CustomerCreationDialog })), {
  ssr: false,
});

const CustomerLifetimeValue = dynamic(() => import("@/components/analytics/CustomerLifetimeValue").then(mod => ({ default: mod.CustomerLifetimeValue })), {
  loading: () => <Skeleton className="h-64 w-full" />,
  ssr: false,
});

const RiskDistribution = dynamic(() => import("@/components/analytics/RiskDistribution").then(mod => ({ default: mod.RiskDistribution })), {
  loading: () => <Skeleton className="h-64 w-full" />,
  ssr: false,
});

const BulkActionsToolbar = dynamic(() => import("@/components/customers/BulkActionsToolbar").then(mod => ({ default: mod.BulkActionsToolbar })), {
  ssr: false,
});

const FilterChips = dynamic(() => import("@/components/customers/FilterChips").then(mod => ({ default: mod.FilterChips })), {
  ssr: false,
});

const SearchHistory = dynamic(() => import("@/components/customers/SearchHistory").then(mod => ({ default: mod.SearchHistory })), {
  ssr: false,
});

const AdvancedSearchBuilder = dynamic(() => import("@/components/customers/AdvancedSearchBuilder").then(mod => ({ default: mod.AdvancedSearchBuilder })), {
  ssr: false,
});

const SavedViews = dynamic(() => import("@/components/customers/SavedViews").then(mod => ({ default: mod.SavedViews })), {
  ssr: false,
});

const ColumnChooser = dynamic(() => import("@/components/customers/ColumnChooser").then(mod => ({ default: mod.ColumnChooser })), {
  ssr: false,
});
...
function CustomersPageContentInternal() {
  ...
  const [minScore, setMinScore] = useState<string>(() => searchParams.get("minScore") || "");
  const [maxScore, setMaxScore] = useState<string>(() => searchParams.get("maxScore") || "");
  ...
  }, [page, pageSize, debouncedSearch, sortBy, order, debouncedRegion, debouncedStatus, debouncedRiskLevel, debouncedEmploymentStatus, debouncedMinScore, debouncedMaxScore, debouncedDateFrom, debouncedDateTo, router]);
  ...
  const filterParams = useMemo(() => {
    ...
    if (debouncedEmploymentStatus !== "all") params.employment_status = debouncedEmploymentStatus;
    ...
    return params;
  }, [debouncedRegion, debouncedStatus, debouncedRiskLevel, debouncedEmploymentStatus, debouncedMinScore, debouncedMaxScore, debouncedDateFrom, debouncedDateTo]);
  ...
  const { data, isLoading, error, refetch, isError } = useCustomersList(queryParams);
  ...
  const handleBulkAction = useCallback(async (action: string, customerIds: string[]) => {
    try {
      const correlationId = getOrCreateCorrelationId();
      // Log bulk action with correlation ID for audit trail
      console.log(`Bulk action: ${action} on ${customerIds.length} customers`, { correlationId });
  ...
  const handleExport = useCallback(async (format: "csv" | "excel" | "pdf" = "csv") => {
    try {
  ...
  const clvData = useMemo(() => {
    try {
      ...
      if (analyticsData?.analytics?.portfolio !== undefined && !analyticsError) {
        // Use real analytics data if available
        const portfolio = analyticsData.analytics.portfolio;
        
        // Validate and extract data safely
        const customers = Array.isArray(portfolio?.customers) ? portfolio.customers : [];
  ...
  const riskData = useMemo(() => {
    try {
      ...
      if (analyticsData?.analytics?.risk !== undefined && !analyticsError) {
        ...
        // Validate and extract data safely
        const categories = Array.isArray(risk?.categories) ? risk.categories : [];
        const trends = Array.isArray(risk?.trends) ? risk.trends : [];
        const scoreDistribution = Array.isArray(risk?.scoreDistribution) ? risk.scoreDistribution : [];
        const regionalData = Array.isArray(risk?.regionalData) ? risk.regionalData : [];
  ...
  const { categories: riskCategories, trends: riskTrends, scoreDistribution, regionalData } = riskData;
  ...
  return (
        ...
        <div className="flex min-h-[400px] items-center justify-center p-4">
          <Card className="w-full max-w-md">
            <CardHeader>
              ...
              <CardDescription>
                An error occurred while loading the customers page. Please try refreshing the page.
              </CardDescription>
            </CardHeader>
            <CardContent>
    ...
    >
      ...
      {isError && error && (
        <Alert variant="destructive">
          ...
          <AlertDescription className="flex items-center justify-between">
            <div>
              <span className="font-semibold">Failed to load customers from API.</span>
      ...
      {!isLoading && !isError && data && (!data.items || !Array.isArray(data.items) || data.items.length === 0) && displayData.total === 0 && (
        <Alert>
          <AlertCircle className="h-4 w-4" />
          <AlertDescription>
      ...
      <Tabs defaultValue="list" className="space-y-6">
        ...
        <TabsContent value="list" className="space-y-6">
            ...
            <Card>
              ...
              <div className="flex items-center gap-2 mb-4">
                ...
                <AdvancedSearchBuilder
                  onSearch={(conditions: SearchCondition[], logic: "AND" | "OR") => {
              ...
              <div className="relative">
                ...
                {search.length >= 2 && searchSuggestions.length > 0 && search !== debouncedSearch && (
                  <div className="absolute z-50 w-full mt-1 bg-white border rounded-md shadow-lg max-h-60 overflow-auto">
                    {searchSuggestions.map((customer: any) => (
                      <div
                        ...
                        onClick={() => {
                          const searchValue = customer.customer_id || customer.id || customer.full_name || "";
                          handleSearch(searchValue);
                        }}
                      >
                        <div className="font-medium">{customer.full_name || customer.name}</div>
                        <div className="text-sm text-muted-foreground">
                          {customer.customer_id || customer.id} • {customer.phone_number || customer.phone}
                        </div>
                      </div>
                    ))}
                  </div>
          ...
          <DashboardSection
            ...
            badge={
              activeFilterCount > 0
                ? { label: `${activeFilterCount} Active`, variant: "secondary" }
                : undefined
            }
          >
            <Card>
                  ...
                  <div className="flex items-center gap-2">
                    <SavedViews
                      ...
                      onLoadView={(view) => {
                        ...
                        if (view.filters.region) setRegion(view.filters.region);
                        if (view.filters.status) setStatus(view.filters.status);
                        if (view.filters.riskLevel) setRiskLevel(view.filters.riskLevel);
                    ...
                    <ColumnChooser
                      columns={[
                        { id: "customer_id", label: "Customer ID", visible: true, order: 0 },
                        { id: "name", label: "Name", visible: true, order: 1, isPII: true },
                        { id: "phone", label: "Phone", visible: true, order: 2, isPII: true },
                        { id: "email", label: "Email", visible: true, order: 3, isPII: true },
                        { id: "credit_score", label: "Credit Score", visible: true, order: 4 },
                        { id: "risk_level", label: "Risk Level", visible: true, order: 5 },
                      ]}
                      onUpdate={(cols) => {
                        // Save column visibility
                        localStorage.setItem("customers_table_columns", JSON.stringify(cols));
                      }}
          ...
          <DashboardSection
            ...
            badge={
              selectedCustomerIds.length > 0
                ? { label: `${selectedCustomerIds.length} Selected`, variant: "secondary" }
                : undefined
            }
          >
            <Card>
        ...
        <TabsContent value="analytics" className="space-y-6">
          ...
          >
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Comprehensive analytics and insights for your customer portfolio
                </CardDescription>
              </CardHeader>
            </Card>
          </DashboardSection>
                      ...
                      <p className="text-2xl font-bold text-red-600">
                        {riskCategories.length > 0
                          ? `${(riskCategories
                              ...
                              riskCategories.reduce((sum, c) => sum + c.count, 0) * 100).toFixed(1)}%`
                          : "0%"}
                      </p>
                    </div>
          ...
          >
            ...
            <Alert variant="destructive">
              ...
              <AlertDescription>
                {analyticsError?.message || "Unknown error occurred while loading analytics data."}
                <div className="mt-2">
                  <Button
                    variant="outline"
                    size="sm"
...
export default function CustomersPage() {
  return (
    <Suspense fallback={<div className="flex items-center justify-center h-96"><div className="text-muted-foreground">Loading customers...</div></div>}>
