  ...
  const handleExport = async (
    ...
    const dataToExport =
      bulk && selectedIds.size > 0
        ? historyData.filter((item: any) =>
            selectedIds.has(item.id || item.prediction_id)
          )
        : historyData;
    ...
    if (format === "csv") {
      const exportData = dataToExport.map((item: any) => ({
        ...
        "Survival Probability": item.survival_probability
          ? `${(item.survival_probability * 100).toFixed(2)}%`
          : "N/A",
      ...
      await exportToCSV(exportData, "default_prediction_history", undefined, {
        ...
        version: "1.0.0",
        filterSummary: bulk
          ? `Bulk export: ${selectedIds.size} items`
          : `Total: ${dataToExport.length} items`,
      });
    } else if (format === "pdf") {
      ...
      <div className="flex items-center justify-between">
        <div>
          <div className="mb-2 flex items-center justify-between">
            ...
            <CacheMetadata
              cacheKey="default-prediction-history"
              variant="compact"
            />
          </div>
                    ...
                    <thead>
                      <tr className="border-b bg-muted/50">
                        <th className="w-12 p-3 text-left text-sm font-medium">
                          <Checkbox
                            checked={
                              selectedIds.size === historyData.length &&
                              historyData.length > 0
                            }
                ...
                {historyResponse && (
                  <Pagination
                    ...
                    totalPages={Math.ceil(
                      (historyResponse.total || historyData.length) / pageSize
                    )}
                    pageSize={pageSize}
