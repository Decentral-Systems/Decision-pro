...
import { DefaultPredictionRequest, DefaultPredictionResponse } from "@/types/prediction";
import { Alert, AlertDescription } from "@/components/ui/alert";
import { Tabs, TabsContent, TabsList, TabsTrigger } from "@/components/ui/tabs";
import { AlertTriangle, User, Users, BarChart3, LineChart, Activity, AlertCircle, Download, Settings, Lightbulb, CheckCircle2, Brain } from "lucide-react";
  ...
  return (
    <div className="space-y-8">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold tracking-tight">Default Prediction</h1>
          ...
          {predictMutation.isError && (
            <Alert variant="destructive" className="mb-6">
              ...
              <AlertDescription className="flex items-center justify-between">
                <div>
                  <span className="font-semibold">Failed to get prediction from API.</span>
                  <p className="text-sm mt-1 text-muted-foreground">
                    Error: {(predictMutation.error as any)?.message || "Unknown error occurred"}
                    {(predictMutation.error as any)?.statusCode && ` (Status: ${(predictMutation.error as any)?.statusCode})`}
                  </p>
                </div>
                <Button
                  variant="outline"
        ...
        <TabsContent value="batch">
          <BatchPredictionForm 
            onComplete={(results) => {
              ...
              console.log("Batch complete:", results, "Correlation ID:", correlationId);
            }}
          />
        </TabsContent>
            ...
            {!result ? (
              <Card>
                <CardContent className="py-12 text-center">
                  ...
                  <p className="text-muted-foreground">
                    Run a prediction first to view analytics. The analytics will be generated based on your prediction results.
                  </p>
                </CardContent>
              </Card>
            ) : (
              <>
                    ...
                    <CardHeader>
                      <div className="flex items-center justify-between">
                        <div>
                          ...
                          <CardDescription>Probability of loan survival over time</CardDescription>
                        </div>
                        <div className="flex items-center gap-2">
                          <Button
                            variant="outline"
                            size="sm"
                            onClick={async () => {
                              const exportData = survivalData.map((d: any) => ({
                                Time: d.time || d.period || d.month,
                                "Survival Probability": d.probability || d.survival_probability,
                              }));
                              await exportToCSV(exportData, "survival_curve", undefined, {
                          ...
                          <Button
                            ...
                            onClick={async () => {
                              ...
                              await exportToPDF(htmlContent, "survival_curve", {
                                includeSignature: true,
                                version: "1.0.0",
                    ...
                    <CardHeader>
                      <div className="flex items-center justify-between">
                        <div>
                          ...
                          <CardDescription>Instantaneous default risk at each time point</CardDescription>
                        </div>
                        <Button
                          variant="outline"
                          size="sm"
                ...
                {sensitivityFactors.length > 0 && scenarios.length > 0 ? (
                  <Card>
                    <CardHeader>
                      <div className="flex items-center justify-between">
                        <div>
                          ...
                          <CardDescription>Impact of input variations on default prediction</CardDescription>
                        </div>
                        <Dialog open={scenarioEditorOpen} onOpenChange={setScenarioEditorOpen}>
                          ...
                          <DialogContent className="max-w-2xl">
                            <DialogHeader>
                              ...
                              <DialogDescription>
                                Create and compare custom scenarios for sensitivity analysis
                              </DialogDescription>
                            </DialogHeader>
                            <div className="space-y-4">
                              <div className="text-sm text-muted-foreground">
                                Base scenario factors: {sensitivityFactors.map((f: any) => f.name).join(", ")}
                              </div>
                              <Button
                                variant="outline"
                                onClick={() => {
                                  setCustomScenarios([...customScenarios, { name: `Scenario ${customScenarios.length + 1}`, factors: {} }]);
                                }}
                              >
                                Add Scenario
                              </Button>
                                ...
                                <div key={idx} className="border rounded p-4 space-y-2">
                                  ...
                                  {sensitivityFactors.map((factor: any) => (
                                    <div key={factor.name} className="flex items-center gap-2">
                                      <Label className="w-32">{factor.name}:</Label>
                                      <Input
                                        type="number"
                                        value={scenario.factors[factor.name] || factor.value}
                                        onChange={(e) => {
                                          const updated = [...customScenarios];
                                          updated[idx].factors[factor.name] = parseFloat(e.target.value);
                                          setCustomScenarios(updated);
                                        }}
                                      />
                                    </div>
                    ...
                    <CardContent>
                      <SensitivityAnalysis
                        ...
                        baseOutcome={result?.expected_value || result?.default_probability || 0}
                  ...
                  <Card>
                    <CardContent className="py-8 text-center text-muted-foreground">
                      Sensitivity analysis data not available in prediction result
                    </CardContent>
                  </Card>
                )}
                ...
                {result && (
                  <Card>
                    <CardHeader>
                      ...
                      <CardDescription>What-if analysis to improve default probability</CardDescription>
                    </CardHeader>
                    <CardContent>
                      <div className="space-y-3">
                        <div className="p-3 border rounded-lg">
                          <div className="flex items-center gap-2 mb-2">
                            ...
                            <span className="font-medium">Reduce loan amount by 10%</span>
                          </div>
                        ...
                        <div className="p-3 border rounded-lg">
                          <div className="flex items-center gap-2 mb-2">
                            <CheckCircle2 className="h-4 w-4 text-green-600" />
                            <span className="font-medium">Extend loan term by 6 months</span>
                          </div>
                          <p className="text-sm text-muted-foreground">
                        ...
                        <div className="p-3 border rounded-lg">
                          <div className="flex items-center gap-2 mb-2">
                            ...
                            <span className="font-medium">Increase interest rate by 2%</span>
                          </div>
                          <p className="text-sm text-muted-foreground">
                      ...
                      <div className="space-y-2">
                        ...
                        <div className="flex items-center justify-between">
                          ...
                          <Badge variant="outline" className="text-xs">
                            {result.correlation_id ? `${result.correlation_id.substring(0, 8)}...` : "N/A"}
                          </Badge>
                        </div>
      ...
      {result && !showAnalytics && (
        <Card className="mt-6">
          <CardHeader>
            ...
            <CardDescription>
              View detailed survival curves, hazard rates, and sensitivity analysis for this prediction
            </CardDescription>
          </CardHeader>
          <CardContent>
