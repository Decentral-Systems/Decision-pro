...
function DisbursementDetailsPageContent() {
  ...
  const { data: disbursementData, isLoading } = useDisbursement(disbursementId || undefined);
  ...
  if (!disbursementData?.data) {
    return (
      <div className="text-center py-12">
        ...
        <Button variant="outline" onClick={() => navigateTo("/loans/disbursements")} className="mt-4">
    ...
    <div className="space-y-8">
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-4">
          <Button variant="outline" onClick={() => navigateTo("/loans/disbursements")}>
            <ArrowLeft className="mr-2 h-4 w-4" />
            Back
          </Button>
          <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <CardContent className="space-y-4">
            <div className="grid grid-cols-2 gap-4 text-sm">
              <div>
                <div className="font-medium text-muted-foreground">Disbursement ID</div>
                <div className="font-mono">{disbursement.disbursement_id}</div>
              </div>
              <div>
              ...
              <div>
                <div className="font-medium text-muted-foreground">Loan Application ID</div>
                <div>{disbursement.loan_application_id}</div>
              </div>
              <div>
                <div className="font-medium text-muted-foreground">Payment Method</div>
                <div>{disbursement.payment_method?.replace(/_/g, " ").toUpperCase()}</div>
              </div>
              <div>
                <div className="font-medium text-muted-foreground">Approved Amount</div>
                <div>
              ...
              <div>
                <div className="font-medium text-muted-foreground">Net Disbursement</div>
                <div>
                  {new Intl.NumberFormat("en-ET", {
                    style: "currency",
                    currency: "ETB",
                  }).format(parseFloat(disbursement.net_disbursement_amount || 0))}
                </div>
              </div>
            </div>
          </CardContent>
      ...
      {disbursement.transaction_reference && (
        ...
        >
          ...
          <CardContent>
            <div className="font-mono text-sm">{disbursement.transaction_reference}</div>
          </CardContent>
          </Card>
        </DashboardSection>
      )}
      ...
      >
        ...
        <CardContent>
          <div className="flex items-center space-x-2">
            ...
            {["pending", "processing"].includes(disbursement.disbursement_status) && (
