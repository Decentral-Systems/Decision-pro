...
function DisbursementsPageContent() {
  ...
  const analytics = useMemo(() => {
    const pending = disbursements.filter(
      (d: any) => d.disbursement_status === "pending"
    ).length;
    const processing = disbursements.filter(
      (d: any) => d.disbursement_status === "processing"
    ).length;
    const completed = disbursements.filter(
      (d: any) => d.disbursement_status === "completed"
    ).length;
    const failed = disbursements.filter(
      (d: any) => d.disbursement_status === "failed"
    ).length;
    const totalAmount = disbursements.reduce(
      (sum: number, d: any) => sum + parseFloat(d.net_disbursement_amount || 0),
      0
    );
    const pendingAmount = disbursements
      ...
      .reduce(
        (sum: number, d: any) =>
          sum + parseFloat(d.net_disbursement_amount || 0),
        0
      );
  ...
  const handleExportCSV = () => {
    const exportData = disbursements.map((d: any) => ({
      ...
      "Requested Date": d.requested_date
        ? format(new Date(d.requested_date), "yyyy-MM-dd")
        : "N/A",
      "Transaction Reference": d.transaction_reference || "N/A",
    }));
    exportToCSV(
      exportData,
      `disbursements_${format(new Date(), "yyyy-MM-dd")}`
    );
    toast({
      title: "Success",
                ...
                <Table>
                  ...
                  <TableBody>
                    {disbursements.map((disb: any) => {
                      ...
                      const requestedDate = disb.requested_date
                        ? new Date(disb.requested_date)
                        : null;
                      ...
                      return (
                        ...
                        >
                          <TableCell className="font-medium">
                            <div className="flex items-center gap-2">
                              ...
                              {isHighPriority && !isUrgent && (
                                <Badge
                                  variant="outline"
                                  className="bg-yellow-500/10 text-xs text-yellow-700 dark:text-yellow-400"
                                >
                                  High Priority
                                </Badge>
                              )}
                          ...
                          <TableCell>
                            {getPaymentMethodLabel(disb.payment_method)}
                          </TableCell>
                          <TableCell>
                            {getStatusBadge(disb.disbursement_status)}
                          </TableCell>
                          <TableCell>
                            <div>
                              {disb.requested_date
                          ...
                          <TableCell>
                            <div className="flex items-center gap-2">
                              <Button
                                ...
                                onClick={() => {
                                  navigateTo(
                                    `/loans/disbursements/${disb.disbursement_id}`
                                  );
                                }}
                              >
      ...
      <Dialog open={processDialogOpen} onOpenChange={setProcessDialogOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Are you sure you want to process this disbursement? This will
          ...
          {selectedDisbursement && (
            <div className="space-y-2">
              ...
              <p className="text-sm">
                ...
                {getPaymentMethodLabel(selectedDisbursement.payment_method)}
              </p>
            </div>
          )}
          <DialogFooter>
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4 text-sm">
                ...
                <div>
                  ...
                  {selectedDisbursement.failed_at
                    ? format(
                        new Date(selectedDisbursement.failed_at),
                        "MMM dd, yyyy HH:mm"
                      )
                    : "N/A"}
                </div>
              ...
              {selectedDisbursement.error_details && (
                <div className="rounded-lg bg-muted p-4">
                  <strong className="mb-2 block text-sm">Error Details:</strong>
