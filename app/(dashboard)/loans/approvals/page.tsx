...
function ApprovalsPageContent() {
  ...
  const { data: applicationData } = useLoanApplication(
    selectedApplication?.loan_application_id || null
  );
  ...
  const handleApprove = async () => {
    ...
    try {
      ...
      toast({
        ...
        description:
          "Your approval request is being processed. This may take a moment...",
        duration: 3000,
      });
  ...
  const handleBulkApprove = async () => {
    ...
    const workflows = approvals.filter((a: any) =>
      selectedWorkflows.has(a.workflow_id)
    );
    const applicationIds = workflows
      .map((w: any) => w.loan_application_id)
      .filter((id: any) => id);
  ...
  const filteredApprovals = approvals.filter((approval: any) => {
    // Stage filter
    if (filterStage !== "all" && approval.current_stage !== filterStage)
      return false;

    // Status filter
    if (filterStatus !== "all" && approval.workflow_status !== filterStatus)
      return false;

    // Search query (customer name, application number)
    if (searchQuery) {
      const query = searchQuery.toLowerCase();
      const matchesCustomer = approval.customer_id
        ?.toLowerCase()
        .includes(query);
      const matchesApplication = approval.application_number
        ?.toLowerCase()
        .includes(query);
    ...
    // Amount range filter
    if (filterAmountMin !== undefined && approval.loan_amount < filterAmountMin)
      return false;
    if (filterAmountMax !== undefined && approval.loan_amount > filterAmountMax)
      return false;
    ...
    if (
      filterRiskLevel !== "all" &&
      approval.risk_level &&
      approval.risk_level !== filterRiskLevel
    )
      return false;
  ...
  const analytics = React.useMemo(() => {
    ...
    const byStage = filteredApprovals.reduce(
      (acc: Record<string, number>, approval: any) => {
    ...
    const byStatus = filteredApprovals.reduce(
      (acc: Record<string, number>, approval: any) => {
    ...
    const totalAmount = filteredApprovals.reduce(
      (sum: number, approval: any) => {
          ...
          <Card>
            ...
            <CardContent>
              ...
              <p className="text-xs text-muted-foreground">
                {analytics.total > 0
                  ? ((analytics.pendingCount / analytics.total) * 100).toFixed(
                      1
                    )
                  : 0}
                % of total
                ...
                <div>
                  ...
                  <Select value={filterStage} onValueChange={setFilterStage}>
                    ...
                    <SelectContent>
                      ...
                      <SelectItem value="final_approval">
                        Final Approval
                      </SelectItem>
                    </SelectContent>
                  </Select>
      ...
      <DashboardSection
        ...
        badge={
          filteredApprovals.length > 0
            ? {
                label: `${filteredApprovals.length} Pending`,
                variant: "warning",
              }
            : undefined
        }
      >
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              {filteredApprovals.length} of {total} applications pending
              approval
            </CardDescription>
          </CardHeader>
          <CardContent>
            {isLoading ? (
              ...
              <div className="py-8 text-center">
                <p className="text-muted-foreground">
                  No pending approvals match the filters
                </p>
              </div>
            ) : (
              <>
                <Table>
                  ...
                  <TableBody>
                    {filteredApprovals.map((approval: any) => {
                      ...
                      const applicationId =
                        approval.loan_application_id || approval.application_id;
                      return (
                          ...
                          >
                            <TableCell onClick={(e) => e.stopPropagation()}>
                              <Checkbox
                                ...
                                onCheckedChange={() =>
                                  toggleWorkflowSelection(approval.workflow_id)
                                }
                              />
                            </TableCell>
                            <TableCell onClick={(e) => e.stopPropagation()}>
                              <Button
                                ...
                                size="sm"
                                onClick={() =>
                                  toggleRowExpansion(approval.workflow_id)
                                }
                              >
                                {isExpanded ? (
                                ...
                                >
                                  ...
                                  <DialogContent className="max-w-2xl">
                                    <DialogHeader>
                                      ...
                                      <DialogDescription>
                                        Provide a reason and rationale for
                                        approval
                                      </DialogDescription>
                                    </DialogHeader>
                                    <div className="space-y-4">
                                      ...
                                      <div>
                                        <Label htmlFor="approve-rationale">
                                ...
                                >
                                  ...
                                  <DialogContent className="max-w-2xl">
                                    ...
                                    <div className="space-y-4">
                                      ...
                                      <div>
                                        <Label htmlFor="reject-rationale">
                                          Decision Rationale (Optional)
                                        </Label>
                                        <Textarea
                                          id="reject-rationale"
                                ...
                                <div className="space-y-4 p-4">
                                  <div className="grid grid-cols-2 gap-4 text-sm md:grid-cols-4">
                                    ...
                                    <div>
                                      ...
                                      <div>
                                        {approval.required_approvals || "N/A"}
                                      </div>
                                    </div>
                                    <div>
                                      <div className="font-medium text-muted-foreground">
                    ...
                    <div className="space-y-4">
                      <div className="grid grid-cols-2 gap-4 text-sm">
                        ...
                        <div>
                          ...
                          <Badge
                            className={getStageBadgeColor(
                              workflowData.data.current_stage
                            )}
                          >
                            {workflowData.data.current_stage
                              ?.replace(/_/g, " ")
                        ...
                        <div>
                          ...
                          <Badge
                            className={getStatusBadgeColor(
                              workflowData.data.workflow_status
                            )}
                          >
                      ...
                      {workflowData.data.workflow_stages &&
                        workflowData.data.workflow_stages.length > 0 && (
                          <div>
                            <div className="mb-2 font-medium text-muted-foreground">
                              Workflow Stages
                            </div>
                            <div className="flex flex-wrap gap-2">
                              {workflowData.data.workflow_stages.map(
                                (stage: string, idx: number) => (
                                  <Badge
                                    key={idx}
                                    variant={
                                      stage === workflowData.data.current_stage
                                        ? "default"
                                        : "outline"
                                    }
                                    className={
                      ...
                      {workflowData.data.workflow_stages && (
                        <WorkflowStageTimeline
                          stages={workflowData.data.workflow_stages.map(
                            (stage: string) => {
                              // Find history entry for this stage
                              const stageHistory = historyData?.data?.find(
                                (h: any) => h.stage === stage
                              );
                              const currentStageIndex =
                                workflowData.data.workflow_stages.indexOf(
                                  workflowData.data.current_stage
                                );
                              const stageIndex =
                                workflowData.data.workflow_stages.indexOf(
                      ...
                      {workflowData.data.workflow_id && (
                        <ConditionalApprovalManager
                          ...
                          conditions={
                            Array.isArray(workflowData.data.conditions)
                              ? workflowData.data.conditions
                              : workflowData.data.conditions
                                ? typeof workflowData.data.conditions ===
                                  "string"
                ...
                <CardContent>
                  <div className="grid grid-cols-2 gap-4 text-sm">
                    ...
                    <div>
                      ...
                      <div>
                        <Badge>
                          {applicationData.data.risk_level || "N/A"}
                        </Badge>
                      </div>
                    </div>
                    <div>
                      ...
                      <div>
                        <Badge
                          className={
                            applicationData.data.nbe_compliant
                              ? "bg-green-500"
                              : "bg-red-500"
                          }
                        >
                      ...
                      <CardContent>
                        <div className="space-y-2">
                          ...
                          {ruleEvaluation.matched_rules &&
                            ruleEvaluation.matched_rules.length > 0 && (
                              <div>
                                <div className="mb-2 font-medium">
                                  Matched Rules (
                                  {ruleEvaluation.matched_rules.length})
                                </div>
                                <div className="max-h-48 space-y-2 overflow-y-auto text-sm">
                                  {ruleEvaluation.matched_rules.map(
                                    (rule: any, idx: number) => (
                                      <div
                                        key={idx}
                    ...
                    <Card>
                      ...
                      <CardContent>
                        <div className="space-y-2">
                          ...
                          {workflowEvaluation.actions &&
                            workflowEvaluation.actions.length > 0 && (
                              <div>
                                <div className="mb-2 font-medium">
                                  Automated Actions (
                                  {workflowEvaluation.actions.length})
                                </div>
                                <div className="space-y-2 text-sm">
                                  {workflowEvaluation.actions.map(
                          ...
                          {workflowEvaluation.matched_rules &&
                            workflowEvaluation.matched_rules.length > 0 && (
                              <div>
                                <div className="mb-2 font-medium">
                                  Matched Workflow Rules (
                                  {workflowEvaluation.matched_rules.length})
                                </div>
                                <div className="max-h-48 space-y-2 overflow-y-auto text-sm">
                                  {workflowEvaluation.matched_rules.map(
                                    (rule: any, idx: number) => (
                                      <div
                                        key={idx}
                                        className="rounded border bg-muted p-3"
        ...
        <DialogContent>
          ...
          <div className="space-y-4">
            <div>
              ...
              <Select
                value={bulkDecision}
                onValueChange={(v: any) => setBulkDecision(v)}
              >
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
