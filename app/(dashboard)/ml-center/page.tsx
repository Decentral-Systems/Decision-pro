...
import { useMLCenterData, useStartTraining, useModelComparison, usePerformanceTrends, useDataDrift } from "@/lib/api/hooks/useML";
...
export default function MLCenterPage() {
  ...
  const { data: comparisonData, isLoading: comparisonLoading } = useModelComparison();
  const { data: trendsData, isLoading: trendsLoading } = usePerformanceTrends("30d", "day");
  ...
  const comparisonTransform = comparisonData && trendsData
  ...
  const { summary: driftSummary, featureDrifts, trends: driftTrends } = driftTransform;
        ...
        <div className="flex min-h-[400px] items-center justify-center p-4">
          <Card className="w-full max-w-md">
            <CardHeader>
              ...
              <CardDescription>
                An error occurred while loading the ML Center. Please try refreshing the page.
              </CardDescription>
            </CardHeader>
            <CardContent>
      ...
      {error && !mlData && (
        <Alert variant="destructive">
          ...
          <AlertDescription className="flex items-center justify-between">
            <div>
              <span className="font-semibold">Failed to load ML Center data from API.</span>
        ...
        <>
          ...
          >
                ...
                <Card>
                  ...
                  <CardContent>
                    <div className="text-2xl font-bold">
                      {mlData.metrics?.active_models || 0} / {mlData.metrics?.total_models || 0}
                    </div>
            ...
            <Card>
              ...
              <CardContent>
                <div className="text-2xl font-bold">
                  {((mlData.metrics?.average_accuracy || 0) * 100).toFixed(1)}%
                </div>
                <p className="text-xs text-muted-foreground">
                  Across all models
                </p>
            ...
            <Card>
              ...
              <CardContent>
                <div className="text-2xl font-bold">
                  {(mlData.metrics?.total_predictions || 0).toLocaleString()}
                </div>
                <p className="text-xs text-muted-foreground">
                    ...
                    >
                      <div className="space-y-2 flex-1">
                        <div className="flex items-center gap-2">
                          ...
                          {model.weight && (
                            <Badge variant="outline">Weight: {model.weight}</Badge>
                          )}
                        ...
                        <div className="grid grid-cols-4 gap-4 text-sm">
                          ...
                          <div>
                            ...
                            <span className="font-medium">{model.auc_roc.toFixed(3)}</span>
                          </div>
                          <div>
                            <span className="text-muted-foreground">F1 Score: </span>
                            <span className="font-medium">{model.f1_score.toFixed(3)}</span>
                          </div>
                          <div>
                            <span className="text-muted-foreground">Last Trained: </span>
                            <span className="font-medium">
                              {safeFormatDate(model.last_trained, "MMM d, yyyy", "Never")}
                            </span>
                          </div>
                        </div>
                      </div>
                      <div className="flex items-center gap-2">
                        <DropdownMenu>
                          ...
                          <DropdownMenuContent align="end">
                            <DropdownMenuLabel>Model Registry</DropdownMenuLabel>
                            ...
                            <DropdownMenuItem onClick={() => {
                              const correlationId = getOrCreateCorrelationId();
                              window.open(`/api/v1/models/${model.model_id}/artifacts?correlation_id=${correlationId}`, '_blank');
                            }}>
                        ...
                        <Button
                          variant="outline"
                          size="sm"
                          onClick={() => handleStartTraining(model.model_name)}
          ...
          {comparisonLoading || trendsLoading ? (
            ...
            <Card>
              <CardContent className="py-12 text-center">
                ...
                <p className="text-muted-foreground">
                  Model comparison data is not available. The API endpoint may not be implemented yet.
                </p>
              </CardContent>
            </Card>
          ) : (
            <AdvancedModelComparison
              ...
              onDeployModel={(model) => console.log("Deploy model:", model)}
            />
          )}
        </TabsContent>
          ...
          {driftLoading ? (
            ...
            <Card>
              <CardContent className="py-12 text-center">
                <AlertCircle className="h-12 w-12 mx-auto mb-4 text-muted-foreground" />
                <p className="text-muted-foreground">
                  Data drift monitoring data is not available. The API endpoint may not be implemented yet.
                </p>
              </CardContent>
            </Card>
          ) : (
        ...
        <TabsContent value="training" className="space-y-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Recent and active training jobs</CardDescription>
            </CardHeader>
            <CardContent>
              <div className="space-y-4">
                {isLoading ? (
                  ...
                  (mlData.recent_training_jobs || []).map((job) => (
                    ...
                    >
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          <h3 className="font-medium">{job.model_name}</h3>
                          <Badge
                            variant={
                        ...
                        <div className="text-sm text-muted-foreground">
                          Started: {safeFormatDate(job.started_at, "PPp", "Unknown")}
                        </div>
                      </div>
                      {job.status === "running" && (
                      ...
                      {job.status === "completed" && job.metrics && (
                        <div className="grid grid-cols-3 gap-4 text-sm">
                          ...
                          <div>
                            <span className="text-muted-foreground">AUC-ROC: </span>
                            <span className="font-medium">{job.metrics.auc_roc.toFixed(3)}</span>
                          </div>
                          <div>
                            <span className="text-muted-foreground">Time: </span>
                            <span className="font-medium">
                              {Math.round(job.metrics.training_time_seconds / 60)} min
                            </span>
                          </div>
                        </div>
                      )}
                      {(job as any).error_message && (
                        <Alert variant="destructive">
                          <AlertDescription>{(job as any).error_message}</AlertDescription>
                        </Alert>
                      )}
                      <div className="flex items-center gap-2">
                        {job.status === "failed" && (
                        ...
                        <Button
                          ...
                          onClick={() => {
                            const correlationId = getOrCreateCorrelationId();
                            window.open(`/api/v1/models/training/${job.job_id}/artifacts?correlation_id=${correlationId}`, '_blank');
                          }}
                        >
                          <Download className="h-4 w-4 mr-2" />
                          Download Artifacts
                        ...
                        <Button
                          ...
                          onClick={() => {
                            const correlationId = getOrCreateCorrelationId();
                            window.open(`/api/v1/models/training/${job.job_id}/logs?correlation_id=${correlationId}`, '_blank');
                          }}
                        >
                        ...
                        {job.model_id && (
                          <Button
                            ...
                            size="sm"
                            onClick={() => {
                              window.open(`/api/v1/models/${job.model_id}/endpoints`, '_blank');
                            }}
                          >
                            <Zap className="h-4 w-4 mr-2" />
                            View Endpoints
                      ...
                      {job.latency_p95 && job.latency_p99 && (
                        <div className="text-xs text-muted-foreground">
                          Latency: P95={job.latency_p95}ms, P99={job.latency_p99}ms
                        </div>
                      )}
                      {job.lineage && (
                        <div className="text-xs text-muted-foreground space-y-1">
                          <div>Code: {job.lineage.code_version || "N/A"}</div>
                          <div>Data: {job.lineage.data_version || "N/A"}</div>
                          <div>Params: {job.lineage.params_hash ? job.lineage.params_hash.substring(0, 8) + "..." : "N/A"}</div>
                        </div>
                      )}
                    </div>
                  ))
          ...
          <Button
            ...
            onClick={async () => {
              ...
              if (comparisonModels.length > 0) {
                ...
                await exportToCSV(exportData, "model_comparison", undefined, {
          ...
          <Button
            ...
            onClick={async () => {
              ...
              if (featureDrifts.length > 0) {
                ...
                await exportToCSV(exportData, "data_drift_report", undefined, {
                  includeSignature: true,
                  version: "1.0.0",
      ...
      <Dialog open={deployDialogOpen} onOpenChange={setDeployDialogOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Configure deployment settings for {selectedModel?.model_name}
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4">
            <div>
              ...
              <Select defaultValue="production">
                ...
                <SelectContent>
                  ...
                  <SelectItem value="shadow">Shadow (no traffic)</SelectItem>
                </SelectContent>
              </Select>
            </div>
            <div>
              <Label>A/B Allocation: {abAllocation[selectedModel?.model_id] || 0}%</Label>
              <Slider
                value={[abAllocation[selectedModel?.model_id] || 0]}
                onValueChange={([value]) => {
                  setAbAllocation({ ...abAllocation, [selectedModel?.model_id]: value });
                }}
                min={0}
                max={100}
                step={5}
