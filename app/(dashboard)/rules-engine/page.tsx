...
import { CustomProductRule, WorkflowRule, RiskAppetiteConfig } from "@/types/rules";
...
export default function RulesEnginePage() {
  ...
  const [approvalWorkflowDialogOpen, setApprovalWorkflowDialogOpen] = useState(false);
  const [ruleTesterDialogOpen, setRuleTesterDialogOpen] = useState(false);
  const [editingRule, setEditingRule] = useState<any>(null);
  const [testRuleType, setTestRuleType] = useState<"product" | "workflow">("product");
  ...
  useEffect(() => {
    ...
    if (savedView && ["table", "grid", "card", "timeline"].includes(savedView)) {
  ...
  const { data: productRulesData, isLoading: productRulesLoading, refetch: refetchProductRules } = useCustomProductRules({
  ...
  const { data: workflowRulesData, isLoading: workflowRulesLoading, refetch: refetchWorkflowRules } = useWorkflowRules({
    is_active: isActiveFilter === "all" ? undefined : isActiveFilter === "active",
    limit: pageSize,
    offset: (workflowPage - 1) * pageSize,
  });
  const { data: riskAppetiteData, isLoading: riskAppetiteLoading, refetch: refetchRiskAppetite } = useRiskAppetiteConfigs({
    is_active: isActiveFilter === "all" ? undefined : isActiveFilter === "active",
    limit: pageSize,
    offset: (riskPage - 1) * pageSize,
  });
  const { data: approvalWorkflowData, isLoading: approvalWorkflowLoading } = useApprovalWorkflowRules();
  ...
  const handleToggleActive = async (ruleId: number, isActive: boolean, type: string) => {
    try {
      if (type === "product") {
        const rule = productRulesData?.rules?.find((r: CustomProductRule) => r.id === ruleId);
        if (!rule) {
          toast({
            title: "Error",
        ...
        const rule = workflowRulesData?.rules?.find((r: WorkflowRule) => r.id === ruleId);
        if (!rule) {
          toast({
            title: "Error",
            description: "Rule not found",
        ...
        const config = riskAppetiteData?.configs?.find((c: RiskAppetiteConfig) => c.id === ruleId);
        if (!config) {
          toast({
            title: "Error",
      ...
      const errorMessage = error?.response?.data?.detail?.message || error?.message || "Failed to toggle active status";
      toast({
        title: "Error",
  ...
  const filteredProductRules = productRulesData?.rules?.filter((rule: CustomProductRule) => {
  ...
  const filteredWorkflowRules = workflowRulesData?.rules?.filter((rule: WorkflowRule) => {
    if (debouncedSearchQuery) {
      const query = debouncedSearchQuery.toLowerCase();
      return (
        rule.rule_name.toLowerCase().includes(query) ||
  ...
  const filteredRiskAppetite = riskAppetiteData?.configs?.filter((config: RiskAppetiteConfig) => {
    if (debouncedSearchQuery) {
      const query = debouncedSearchQuery.toLowerCase();
      return (
  ...
  const stats = useMemo(() => {
    ...
    const workflowRules = workflowRulesData?.rules || filteredWorkflowRules || [];
    ...
    return {
      ...
      activeProductRules: productRules.filter((r: CustomProductRule) => r.is_active).length,
      totalWorkflowRules: workflowRulesData?.total ?? workflowRules.length,
      activeWorkflowRules: workflowRules.filter((r: WorkflowRule) => r.is_active).length,
      totalRiskConfigs: riskAppetiteData?.total ?? riskConfigs.length,
      activeRiskConfigs: riskConfigs.filter((c: RiskAppetiteConfig) => c.is_active).length,
    };
  }, [productRulesData, filteredProductRules, workflowRulesData, filteredWorkflowRules, riskAppetiteData, filteredRiskAppetite]);
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">
            Configure and manage decision rules for credit scoring and loan approval
          </p>
        </div>
        <div className="flex gap-2">
          <Dialog open={visualRuleBuilderOpen} onOpenChange={setVisualRuleBuilderOpen}>
          ...
          <Dialog open={enhancedRuleTesterOpen} onOpenChange={setEnhancedRuleTesterOpen}>
            <DialogTrigger asChild>
              <Button variant="outline" size="sm">
            ...
            <DialogContent className="max-w-6xl max-h-[90vh] overflow-hidden">
              <DialogHeader>
                ...
                <DialogDescription>
                  Advanced rule testing with batch processing, historical data, and impact analysis
                </DialogDescription>
              </DialogHeader>
              <div className="h-[70vh] overflow-auto">
                <EnhancedRuleTester onClose={() => setEnhancedRuleTesterOpen(false)} />
              </div>
            </DialogContent>
          </Dialog>
          
          <Button onClick={() => setRuleTesterDialogOpen(true)} variant="outline" size="sm">
      ...
      >
        ...
        <CardContent>
          ...
          >
            ...
            <div className="flex gap-2 flex-wrap items-center mb-4 pb-4 border-b">
                ...
                <>
                  <div className="flex items-center gap-1">
                    <ViewSwitcher
                      ...
                      availableViews={
                        activeTab === "approval" ? ["table"] : ["table", "grid", "card", "timeline"]
                      }
                    />
                    <InfoButton section="views" tooltip="Learn about view options" />
                  </div>
                  {viewMode === "table" && (
                    <Button
                      variant="outline"
                      ...
                      onClick={() => {
                        ...
                        localStorage.setItem("rules-compact-view", String(!compactView));
                      }}
              ...
              <div className="relative flex items-center gap-1">
                ...
                <InfoButton section="filters" tooltip="Learn about filters and search" />
              </div>
              {activeTab === "product" && (
              ...
              <Button
                onClick={() => {
                  ...
                  else if (activeTab === "workflow") handleCreateRule("workflow");
                  else if (activeTab === "risk") handleCreateRule("risk");
                }}
              >
                <Plus className="h-4 w-4 mr-2" />
                Create {activeTab === "product" ? "Product" : activeTab === "workflow" ? "Workflow" : "Risk"} Rule
              </Button>
            </div>
            ...
            <div className="mb-4 flex items-center gap-2">
              ...
              <div className="flex items-center gap-1">
                {activeTab === "product" && (
                  <InfoButton section="product-rules" tooltip="Learn about Product Rules" />
                )}
                {activeTab === "workflow" && (
                  <InfoButton section="workflow-rules" tooltip="Learn about Workflow Rules" />
                )}
                {activeTab === "risk" && (
                  <InfoButton section="risk-appetite" tooltip="Learn about Risk Appetite" />
                )}
                {activeTab === "approval" && (
                  <InfoButton section="approval-workflow" tooltip="Learn about Approval Workflow" />
                )}
                <InfoButton section="overview" tooltip="View complete documentation" />
              </div>
            </div>
            ...
            <TabsContent value="product" className="space-y-4">
              {productRulesLoading ? (
                ...
                <RulesEmptyState
                  ruleType="product"
                  hasFilters={
                    !!debouncedSearchQuery || productTypeFilter !== "all" || isActiveFilter !== "all"
                  }
                ...
                <>
                  {viewMode === "table" && (
                    <CustomProductRulesTable
                      ...
                      onDuplicate={(rule) => handleDuplicateRule(rule, "product")}
                      onEvaluate={(rule) => handleTestRule(rule, "product")}
                      onToggleActive={(id, isActive) => handleToggleActive(id, isActive, "product")}
                    />
                  )}
                  {viewMode === "grid" && (
                    <RulesGridView
                  ...
                  {productRulesData && (productRulesData.total || 0) > pageSize && (
                    <div className="mt-4">
                      <PaginationControls
                        currentPage={productPage}
                        totalPages={productTotalPages}
                        pageSize={pageSize}
                        totalItems={productRulesData.total || filteredProductRules.length}
                        onPageChange={(page) => {
                          setProductPage(page);
                ...
                <>
                  {viewMode === "table" && (
                    <WorkflowRulesTable
                      ...
                      onDuplicate={(rule) => handleDuplicateRule(rule, "workflow")}
                      onEvaluate={(rule) => handleTestRule(rule, "workflow")}
                      onToggleActive={(id, isActive) => handleToggleActive(id, isActive, "workflow")}
                      compact={compactView}
                    />
                  )}
                  {viewMode === "grid" && (
                  ...
                  {workflowRulesData && (workflowRulesData.total || 0) > pageSize && (
                    <div className="mt-4">
                      <PaginationControls
                        currentPage={workflowPage}
                        totalPages={workflowTotalPages}
                        pageSize={pageSize}
                        totalItems={workflowRulesData.total || filteredWorkflowRules.length}
                        onPageChange={(page) => {
              ...
              {riskAppetiteLoading ? (
                ...
                <>
                  <RiskAppetiteTable
                    ...
                    onDuplicate={(config) => handleDuplicateRule(config, "risk")}
                    onToggleActive={(id, isActive) => handleToggleActive(id, isActive, "risk")}
                  />
                  {riskAppetiteData && (riskAppetiteData.total || 0) > pageSize && (
                    <div className="mt-4">
                      <PaginationControls
                        currentPage={riskPage}
                        totalPages={riskTotalPages}
                        pageSize={pageSize}
                        totalItems={riskAppetiteData.total || filteredRiskAppetite.length}
                        onPageChange={(page) => {
                          setRiskPage(page);
                          window.scrollTo({ top: 0, behavior: "smooth" });
                        }}
            ...
            <TabsContent value="approval" className="space-y-4">
              <div className="flex justify-between items-center mb-4">
                <div>
                  <h3 className="text-lg font-semibold">Approval Workflow Levels</h3>
              ...
              ) : approvalWorkflowData && Array.isArray(approvalWorkflowData) && approvalWorkflowData.length > 0 ? (
                <ApprovalWorkflowTable levels={approvalWorkflowData} isLoading={approvalWorkflowLoading} />
              ) : (
                <div className="text-center py-12">
                  <p className="text-muted-foreground text-lg">No approval workflow levels found</p>
                  <p className="text-muted-foreground text-sm mt-2">
                    Click "Edit Approval Levels" to configure approval workflow
                  </p>
                </div>
              )}
            </TabsContent>
